...
import { PropertiesData } from '@/lib/types'
...
export const metadata: Metadata = {
  title: 'Properties | Vacation Rental',
...
async function getProperties() {
  try {
    ...
    const { data } = await client.raw(GET_PROPERTIES, { first: 50 })
    return data?.nodeProperties?.nodes || []
  } catch (error) {
      ...
      <section className="relative bg-primary-900 text-white py-24 md:py-32 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center pt-8">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold font-display mb-6">Properties</h1>
            <p className="text-xl text-primary-100 max-w-3xl mx-auto">Explore our properties.</p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {items.length === 0 ? (
            <div className="text-center py-12">
              <h2 className="text-2xl font-semibold text-gray-600 mb-2">No Properties Yet</h2>
              <p className="text-gray-500">
                Properties will appear here once content is imported.
              </p>
            </div>
          ) : (
